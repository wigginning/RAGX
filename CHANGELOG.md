...
Contract-breaking changes are marked **BREAKING**; they are only permitted before `v1.0.0` (which freezes the SPI and REST contracts).
...
Highlights: multi-tenant audit, Helm chart, full MCP support, complete E2E suite; all internal quality gates green and a security review passed.
...
- **Runnable L4 evaluation gate** (`c492419`) — `scripts/eval_l4.py` +
  ...
  install extra (ragas/deepeval/datasets) and a `build_evaluator()` factory. The gate itself (run_eval, baseline comparison,
...
- **CI aligned with the L1–L4 gates** (`…`) — GitHub Actions now run the
  ...
  omitted — thresholds in `pyproject.toml`). New `nightly.yml` runs the full
